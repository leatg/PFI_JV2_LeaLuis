...
namespace ECM.Controllers
{
    ...
    public class BaseFirstPersonController : BaseCharacterController
    {
        ...
        protected virtual float GetTargetSpeed()
        {
            ...
            StaminaComponent staminaCompo = GetComponent<StaminaComponent>();
            runSpeedMultiplier = 1;
            if (!(staminaCompo.GetStamina() > 0)) return run ? targetSpeed * runSpeedMultiplier : targetSpeed;
            if (staminaCompo.CanRun()) runSpeedMultiplier = staminaCompo.CanRunFast() ? 1.55f : 1.25f;
